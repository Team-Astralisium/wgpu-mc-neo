...
class FloatWidget(x: Int, y: Int, width: Int, private val option: FloatOption) :
    ...
    private fun calculateValue(mouseX: Int) {
        ...
        if (mouseX < 0) option.set(option.min)
            ...
            option.set(Math.round(value / option.step) * option.step)
        }
    }
    ...
    private fun getHandleX(): Int {
        val delta = (option.get() - option.min) / (option.max - option.min)
